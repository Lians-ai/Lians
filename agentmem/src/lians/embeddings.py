...
class BgeOnnxProvider(EmbeddingProvider):
    ...
    The provider is deliberately opt-in. It accepts only an artifact produced
    by ``lians-bge-onnx-export`` from the pinned BAAI revision, validates every
    byte before the first inference, and keeps document embeddings compatible
    with existing 1024-dimensional BGE stores. Model loading remains lazy.
    """
    ...
    def _load(self) -> _BgeOnnxRuntime:
        ...
        if (
            len(output_shape) != 3
            or output_shape[-1] != BGE_ONNX_EMBEDDING_DIMENSION
        ):
            raise ValueError(
    ...
    def _encode_sync(runtime: _BgeOnnxRuntime, texts: List[str]) -> List[List[float]]:
        ...
        with runtime.inference_lock:
            ...
            output = np.asarray(
                runtime.session.run(["last_hidden_state"], feed)[0]
            )
