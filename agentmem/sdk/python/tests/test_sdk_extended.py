...
import pytest
from lians import LiansClient, LocalLiansClient
...
class TestSyncHTTPClientExtended:
    ...
    def test_batch_add_delegates(self):
        ...
        client._async.batch_add.assert_called_once_with(memories, idempotency_key=None)
    ...
    def test_confirm_supersession_delegates(self):
        ...
        result = client.confirm_supersession(
            "abc", expected_superseded_by="successor", reviewer_note="ok"
        )

        client._async.confirm_supersession.assert_called_once_with(
            memory_id="abc",
            expected_superseded_by="successor",
            reviewer_note="ok",
        )
        assert result == expected
        client.close()
    ...
    def test_reject_supersession_delegates(self):
        ...
        result = client.reject_supersession(
            "abc", expected_superseded_by="successor", reviewer_note="wrong"
        )
    ...
    def test_audit_export_delegates(self):
        ...
        client._async.audit_export.assert_called_once_with(
            ...
            limit=10_000,
            verify=True,
            after_chain_position=None,
            through_chain_position=None,
        )
        assert result == expected
        client.close()
