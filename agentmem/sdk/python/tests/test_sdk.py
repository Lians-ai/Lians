...
class TestLocalClient:
    ...
    def test_erase_removes_content(self):
        ...
        assert result["snapshot"]["memories"] == 1
    ...
    def test_erase_audit_trail_preserved(self):
        ...
        with LocalLiansClient() as mem:
            ...
            reconstruction = mem.reconstruct(agent_id="a", as_of=T2)
            audit = mem.audit_export()

        assert "add" in {e["op"] for e in reconstruction["event_trail"]}
        # The privacy-controller event is namespace-scoped rather than
        # attributed to the memory's agent, so verify it in the audit export.
        assert "subject_erasure_requested" in {e["op"] for e in audit["events"]}
