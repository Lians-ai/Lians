...
from benchmarks.codex_sol_installed_plugin_ab import (  # noqa: E402
...
class _FakeAppServer:
        ...
        *,
        ...
        if method == "thread/start":
            ...
            return {
                ...
                "sandbox": {
                    "type": "workspaceWrite",
                    "writableRoots": copied["runtimeWorkspaceRoots"],
                    "networkAccess": False,
        ...
        if method == "turn/start":
            ...
            receipt = state / ("candidate-receipts.jsonl" if candidate else "baseline-receipts.jsonl")
    ...
    def _queue_turn(self, thread_id: str, turn_id: str, *, candidate: bool) -> None:
        ...
        total_input = 90_000 if candidate and self.high_candidate_usage else (10_000 if candidate else 30_000)
...
def test_preflight_precedes_model_and_requests_pin_the_sol_profile(tmp_path: Path) -> None:
    ...
    assert len(thread_requests) == len(turn_requests) == 2
    for thread, turn in zip(thread_requests, turn_requests, strict=True):
        ...
        assert len(thread["runtimeWorkspaceRoots"]) == 2
        ...
        assert turn["runtimeWorkspaceRoots"] == thread["runtimeWorkspaceRoots"]
        assert turn["sandboxPolicy"] == {
            "type": "workspaceWrite",
            "writableRoots": thread["runtimeWorkspaceRoots"],
            "networkAccess": False,
            "excludeTmpdirEnvVar": True,
            "excludeSlashTmp": True,
        }
...
def test_raw_provider_usage_is_aggregated_and_qualified_artifacts_are_sanitized(tmp_path: Path) -> None:
    config, plugin = _fixture(tmp_path)
    server = _FakeAppServer(plugin)
    ...
    assert all(run["raw_stdout_artifact"] == f"{run['label']}.stdout.jsonl" for run in report["runs"])
    rendered = json.dumps(report)
    assert str(tmp_path) not in rendered
...
def test_plugin_inventory_prefers_the_versioned_installed_cache(
    ...
    assert plugin.bootstrap_sha256 == hashlib.sha256(
        (cache / "scripts" / "bootstrap.py").read_bytes()
    ).hexdigest()
