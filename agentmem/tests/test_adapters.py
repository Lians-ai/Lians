...
from lians.adapters import get_adapter, register_adapter
from lians.adapters.finance import FinanceAdapter
from lians.adapters.passthrough import PassthroughAdapter
from lians.adapters.healthcare import HealthcareAdapter
from lians.adapters.legal import LegalAdapter
from lians._types import (
...
def test_finance_adapter_structured_keys():
    ...
    assert "period" in adapter.structured_keys
    assert "quarter" in adapter.structured_keys
    assert adapter.key_aliases("period") == ["period", "quarter"]
...
def test_finance_adapter_implements_protocol():
    from lians.adapters import DomainAdapter
...
def test_get_adapter_returns_finance_by_default(monkeypatch):
    from lians.adapters import _registry
    _registry.clear()
