...
ProjectPreparer = Callable[[InstalledPlugin, Path, Path, Sequence[Mapping[str, Any]]], PreparedProjects]
...
def _plugin_from_document(document: Mapping[str, Any]) -> InstalledPlugin:
    ...
    matches = [item for item in installed if isinstance(item, Mapping) and item.get("pluginId") == PLUGIN_ID]
    if len(matches) != 1:
    ...
    if all(isinstance(value, str) and value for value in (marketplace_name, plugin_name, version)):
        ...
        for home in homes:
            candidate = home / "plugins" / "cache" / str(marketplace_name) / str(plugin_name) / str(version)
    ...
    if manifest_doc.get("name") != "lians-memory" or manifest_doc.get("version") != item.get("version"):
...
def _project_binding(bootstrap: Any, plugin: InstalledPlugin, project_root: Path) -> tuple[Any, Path, dict[str, str]]:
    try:
        ...
        if not isinstance(artifact, str) or not bootstrap.validate_bge_artifact_directory(Path(artifact)):
...
def _seed_candidate(
    ...
    return {
        ...
        "embedding_provider": "bge-onnx",
        "bge_artifact_sha256": hashlib.sha256(str(reported["bge_artifact_dir"]).encode()).hexdigest(),
...
def prepare_projects(
    ...
    for receipt in (Path(baseline_env["LIANS_CODEX_HOOK_RECEIPT"]), Path(candidate_env["LIANS_CODEX_HOOK_RECEIPT"])):
        if receipt.exists():
...
def _runtime_writable_roots(spec: RunSpec) -> list[str]:
    # The model's working tree and the plugin's single project-scoped state
    # directory are the only writable locations. The installed runtime and
    # model artifacts remain read-only.
    return [str(spec.cwd.resolve()), str(spec.data_home.resolve())]


def _thread_start_params(spec: RunSpec) -> dict[str, Any]:
    writable_roots = _runtime_writable_roots(spec)
    return {
        "model": MODEL,
...
def _validate_hook_preflight(
    ...
    *,
    ...
    if not isinstance(data, list) or len(data) != len(cwds):
        raise BenchmarkError("hooks/list did not return exactly one entry for each A/B working directory")
    ...
    for cwd_key in expected_by_key:
        ...
        if entry.get("warnings") != [] or entry.get("errors") != []:
            raise BenchmarkError("hooks/list reported warnings or errors for an A/B working directory")
        hooks = entry.get("hooks")
        if not isinstance(hooks, list):
            raise BenchmarkError("hooks/list omitted hook metadata")
        lians = [hook for hook in hooks if isinstance(hook, Mapping) and hook.get("pluginId") == PLUGIN_ID]
        if len(lians) != 2:
        ...
        if set(by_event) != set(EXPECTED_HOOK_EVENTS):
            raise BenchmarkError("installed Lians handlers were not exactly SessionStart and UserPromptSubmit")
        for event_name, hook in by_event.items():
            if hook.get("enabled") is not True or hook.get("trustStatus") != "trusted":
                raise BenchmarkError(f"installed Lians {event_name} handler is not enabled and trusted")
            if hook.get("source") != "plugin" or hook.get("handlerType") != "command":
                raise BenchmarkError(f"installed Lians {event_name} handler has the wrong source or type")
            source_path = hook.get("sourcePath")
            if not isinstance(source_path, str) or _path_key(source_path) != expected_source:
                raise BenchmarkError(f"installed Lians {event_name} handler is not from the measured cache root")
            key = hook.get("key")
            if not isinstance(key, str) or not key.startswith(f"{PLUGIN_ID}:") or not key.endswith(
                expected_key_suffix[event_name]
            ):
            ...
            if not isinstance(current_hash, str) or re.fullmatch(r"sha256:[0-9a-f]{64}", current_hash) is None:
...
def _preflight_app_server(
    ...
    *,
    ...
    return _validate_hook_preflight(hooks, cwds=cwds, plugin=plugin)
...
def _validate_thread_start(
    ...
    *,
    ...
    if not isinstance(result.get("cwd"), str) or _path_key(str(result["cwd"])) != _path_key(spec.cwd):
    ...
    expected_roots = {_path_key(path) for path in _runtime_writable_roots(spec)}
    actual_roots = sandbox.get("writableRoots") if isinstance(sandbox, Mapping) else None
    if (
        not isinstance(sandbox, Mapping)
        or sandbox.get("type") != "workspaceWrite"
        or sandbox.get("networkAccess") is not False
        or not isinstance(actual_roots, list)
        or {_path_key(str(path)) for path in actual_roots} != expected_roots
    ):
        raise BenchmarkError(f"{spec.label} app-server changed the bounded workspace-write sandbox")
    ...
    if thread.get("ephemeral") is not True or thread.get("modelProvider") != EXPECTED_MODEL_PROVIDER:
        raise BenchmarkError(f"{spec.label} thread/start did not create the requested ephemeral OpenAI thread")
...
def _artifact_event(message: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    if method in {"hook/started", "hook/completed"}:
        ...
        safe_run = {
            ...
            if isinstance(run.get(key), (str, int, float, bool, type(None)))
        }
        source_path = run.get("sourcePath")
        if isinstance(source_path, str):
            safe_run["sourcePathSha256"] = _sha256_bytes(source_path.encode("utf-8"))
        entries = run.get("entries")
        if isinstance(entries, list):
            safe_run["entries"] = [
                {
                    "kind": entry.get("kind"),
                    "text": _stderr_tail(str(entry.get("text", "")).encode("utf-8")),
                }
    ...
    if method in {"item/started", "item/completed", "rawResponseItem/completed"}:
        ...
        safe_item: dict[str, Any] = {
            key: item[key]
            for key in ("id", "call_id", "type", "phase", "status", "server", "namespace", "tool", "name")
...
def _complete_usage(
    ...
    *,
    ...
    for event in completed:
        ...
        if not isinstance(raw, Mapping) or any(name not in raw for name in provider_names):
...
def _receipt_violations(spec: RunSpec, receipt: Mapping[str, Any]) -> list[str]:
    ...
    if spec.mode == "candidate":
        ...
        if isinstance(tokens, bool) or not isinstance(tokens, int) or not 1 <= tokens <= MAX_CONTEXT_TOKENS:
...
def _parse_run(
    ...
    *,
    ...
    if invocation.returncode != 0:
        raise BenchmarkError(f"{spec.label} exited {invocation.returncode}: {_stderr_tail(invocation.stderr)}")
    events = _parse_events(invocation.stdout, spec.label)
    raw_artifact = _write_raw(raw_dir, spec, invocation.stdout) if raw_dir else None
    ...
    return {
        ...
        "raw_stdout_artifact": raw_artifact,
...
def _profile(plugin: InstalledPlugin, config: BenchmarkConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "sandbox": {
            ...
            "writable_roots": "isolated arm plus Lians data home",
        ...
        "baseline": {"context": "complete LOCOMO conversation", "project_memory": "fresh and empty"},
        "candidate": {
    ...
    *,
    ...
    if dry_run:
        ...
        report["planned_preflight"] = {
            "transport": "persistent codex app-server stdio JSON-RPC",
            "sequence": ["initialize", "initialized", "hooks/list"],
    ...
    with tempfile.TemporaryDirectory(prefix="lians-installed-sol-") as temporary:
        ...
        try:
            ...
            for sequence, (mode, repetition) in enumerate(order, start=1):
                prompt = case["candidate_prompt"] if mode == "candidate" else case["baseline_prompt"]
                spec = _make_spec(
                    config,
                    prepared,
                    sequence=sequence,
    ...
    neutral_baseline = sum(float(run["estimated_sol_credits_all_input_uncached"]) for run in baseline_runs)
    neutral_candidate = sum(float(run["estimated_sol_credits_all_input_uncached"]) for run in candidate_runs)
    ...
    report["verdict"] = {
        ...
        "all_runs_no_tools_or_delegation": all(not run["tool_calls"] and not run["delegation_evidence"] for run in runs),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        report = run_benchmark(config, dry_run=not args.live)
