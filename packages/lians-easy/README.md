...
```bash
python -m lians_easy install --clients claude,cursor,cline,opencode --yes
python -m lians_easy doctor --json
```

Supported targets are Claude Desktop, Cursor, Windsurf, Gemini CLI, Codex,
Cline CLI, and OpenCode. Cline uses its documented CLI settings file at
