...
def _antigravity_plugin_config(
    ...
    *,
    ...
    backups = [
        candidate
        for candidate in (
            _backup(manifest_path, on_created=on_backup),
            _backup(mcp_path, on_created=on_backup),
            _backup(registry_path, on_created=on_backup),
        )
...
def _client_paths(key: str, target: ClientTarget, home: Path) -> list[Path]:
    ...
    if key == "antigravity":
        paths.extend(
            [
                target.config_path.parent / "plugin.json",
                _antigravity_registry_path(home),
            ]
        )
    ...
    *,
    ...
    if key in {"antigravity", "claude", "codex", "gemini"}:
        ...
        if hook_backup is not None:
            item["backups"].append(str(hook_backup))
    if key == "cursor":
...
def _uninstall_unlocked(keys: list[str], *, home: Path | None = None) -> dict[str, Any]:
    ...
    for key in keys:
        ...
        if target.kind == "antigravity_plugin":
            ...
            hook_path = _hook_path(key, home)
            if hook_path.exists():
                hook_backup = _antigravity_hook_config(hook_path, remove=True)
                item["hook_backup"] = str(hook_backup) if hook_backup else None
                if hook_backup is not None:
                    item["backups"].append(str(hook_backup))
                    item["status"] = "removed"
            results.append(item)
            continue
        if not target.config_path.exists() and key not in {
            "claude",
            "codex",
            "gemini",
        }:
        ...
        item = {
            ...
            "backups": [str(item_backup) for item_backup in backups],
        }
        if key in {"antigravity", "claude", "codex", "gemini"}:
            hook_path = _hook_path(key, home)
            if hook_path.exists():
