...
A controlled checkout-hook ABBA run on one LOCOMO question produced four exact
answers and a 2.035x same-budget result (+103.51%) from estimated Sol credits.
The normal installed-plugin loader was not exercised, the candidate was slower
end to end, and this host has no accepted installed-plugin economics result.
Treat usage extension as workload-dependent, not as a guaranteed Codex quota
increase. See the
[controlled evidence](../../docs/benchmarks/codex-sol-ultra-checkout-hook-usage-evidence-2026-08-08.json).
