...
- owns the two documented `lians-reviewer-fixture-v1` records and only the disposable records created by the published cases; and
- can restore either fixed record if an unrelated operation changes it.
...
Browser login for the dedicated reviewer account passed in the production OAuth E2E, Auth0 displayed its latest login at `2026-08-10T03:40:58Z`, and the two public synthetic fixed records were provisioned and verified live. Developer-mode rehearsal and secure portal credential delivery remain pending.
...
Use [`test-cases.json`](./test-cases.json) as the source of truth. It contains exactly five positive and three negative cases. Before each independent case, verify or restore only the `architecture-current` and `region-current` records. Their two documented UUIDs must identify active records owned by the demo account and created through the hosted MCP surface.

The two UUIDs published in `test-cases.json` are verified live references for public synthetic reviewer records. Do not substitute private canary references or content. If either fixed record is reprovisioned, update its published reference. `positive-4-confirmed-forget` does not use a fixed deletion target: it creates a disposable Atlas note without an idempotency key, uses the returned UUID for confirmed removal, and retries that exact `forget_memory` call, so there is no deleted fixed record to reset.

Recommended run order:

1. Run the no-token endpoint check, then complete OAuth with the reviewer account.
2. Verify the two fixed fixture records and run each positive case independently.
3. Verify the two fixed fixture records and run each negative case independently.
4. For `positive-4-confirmed-forget`, verify that `remember` omits `idempotency_key`, capture its returned UUID, confirm removal with the up-to-5-day encrypted provider backup disclosure, and use that UUID for `forget_memory`.
5. Verify the first removal reports `forgotten` with `memories_erased: 1` and an exact retry reports `not_found` with `memories_erased: 0`; do not reset or recreate a fixed deletion target.
...
- `forget_memory` is not called without current explicit confirmation and can remove only one matching active hosted record in the signed-in tenant; user-facing copy discloses that encrypted provider backups may retain a recoverable copy for up to 5 days.
- Secret storage, silent whole-chat capture, and quota-bypass claims are declined without a tool call.

## Evidence to capture

For each case, record the case ID, pass/fail status, discovered tool name, structured-result shape, expected side effect, and review timestamp. For active-storage removal, record the returned status and erase count plus the safe `not_found`/`0` retry. Record endpoint-checker JSON status and the package version.
...
After a review session, clear any injected bearer-token environment value, verify that no `positive-4` disposable record remains active, restore either of the two fixed records only if an unrelated operation changed it, close or revoke the session as required, and rotate temporary reviewer credentials on the operator's schedule. Do not preserve tokens in shell history or captured output.
